-- Fix Journey record policy resolution for legacy Experience captures.
-- PostgreSQL has no max(uuid) aggregate. The fallback path already requires
-- exactly one owner-scoped match, so resolve the UUID with a deterministic
-- single-row SELECT instead.
