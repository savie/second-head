-- P5D-003 — Recovery restores persistent SH identity state.
-- Recovery keeps the canonical SH_ID / ACCOUNT_ID identity anchors and restores
-- the mutable persistent SH state captured in identity_root.
...
begin
 ...
 select jsonb_build_object(
   'identity_root', jsonb_build_object(
     'sh_id',s.sh_id,'account_id',s.account_id,'sh_type',s.sh_type,
     'is_primary',s.is_primary,'canonical_name',s.canonical_name,
     'creator_ref',s.creator_ref,'status',s.status,'metadata',s.metadata,
     'version',s.version
   ),
...
begin
 ...
 set sh_type = coalesce(v_snapshot.manifest->'identity_root'->>'sh_type', sh_type),
     is_primary = coalesce((v_snapshot.manifest->'identity_root'->>'is_primary')::boolean, is_primary),
     canonical_name = v_snapshot.manifest->'identity_root'->>'canonical_name',
     creator_ref = v_snapshot.manifest->'identity_root'->>'creator_ref',
     status = coalesce(v_snapshot.manifest->'identity_root'->>'status', status),
     metadata = coalesce(v_snapshot.manifest->'identity_root'->'metadata', metadata),
     version = coalesce((v_snapshot.manifest->'identity_root'->>'version')::integer, version),
     updated_at = now()
 where sh_id=v_sh_id and account_id=current_account_id();
