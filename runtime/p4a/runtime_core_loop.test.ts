...
Deno.test('P4A-001 resolves existing SH identity and preserves it through model response', async () => {
  ...
  const runtime = createRuntimeCoreLoop({
    ...
    modelAdapter: { async generate(request) { const context = request.context as { identity: { sh_id: string } }; calls.push(`model:${context.identity.sh_id}`); return { output: { type: 'text', content: 'ok' } }; } },
