...
begin
  ...
  if p_event_type not in ('RUNTIME_REQUEST','RUNTIME_RESPONSE','RUNTIME_MEMORY_DECISION','TOOL_INVOCATION','RUNTIME_ACTION') then
...
begin
  select * into v_identity from public.resolve_identity();
  if v_identity.account_id is null or v_identity.sh_id is null then
    raise exception 'HIGH_RISK_REJECTED: authenticated identity could not be resolved';
  end if;
  ...
  if not exists (
    select 1 from public.recovery_snapshots rs
    where rs.snapshot_id = p_target_id and rs.account_id = v_identity.account_id and rs.sh_id = v_identity.sh_id
  ) then
    raise exception 'HIGH_RISK_REJECTED: recovery snapshot is not accessible';
  end if;
  insert into public.runtime_high_risk_confirmations(
    action_id, account_id, sh_id, actor_id, operation, risk, status, target_id, title, description
  ) values (
    p_action_id, v_identity.account_id, v_identity.sh_id, auth.uid(), p_operation, 'HIGH', 'PENDING',
    p_target_id, coalesce(nullif(btrim(p_title),''),'Confirm recovery restore'),
    coalesce(nullif(btrim(p_description),''),'Restore this SH recovery snapshot.')
  ) returning confirmation_id into v_confirmation_id;
  perform public.runtime_record_audit(v_identity.sh_id,'RUNTIME_ACTION','SUCCESS',jsonb_build_object(
    'action','HIGH_RISK_CONFIRMATION_CREATED','action_id',p_action_id,'operation',p_operation,'confirmation_id',v_confirmation_id,'target_id',p_target_id
  ));
...
declare
  v_identity record;
  v_row public.runtime_high_risk_confirmations%rowtype;
begin
  select * into v_identity from public.resolve_identity();
  ...
  select * into v_row from public.runtime_high_risk_confirmations
    where confirmation_id=p_confirmation_id and account_id=v_identity.account_id and sh_id=v_identity.sh_id for update;
  ...
  if v_row.expires_at <= now() then
    update public.runtime_high_risk_confirmations set status='EXPIRED' where confirmation_id=p_confirmation_id;
    raise exception 'HIGH_RISK_CONFIRMATION_REJECTED: confirmation expired';
  end if;
  ...
  perform public.runtime_record_audit(v_identity.sh_id,'RUNTIME_ACTION','SUCCESS',jsonb_build_object(
    'action','HIGH_RISK_CONFIRMATION_CONFIRMED','action_id',v_row.action_id,'operation',v_row.operation,'confirmation_id',p_confirmation_id,'target_id',v_row.target_id
  ));
...
begin
  ...
  if v_row.expires_at <= now() then
    update public.runtime_high_risk_confirmations set status='EXPIRED' where confirmation_id=p_confirmation_id;
    raise exception 'HIGH_RISK_EXECUTION_REJECTED: confirmation expired';
  end if;
  if v_row.actor_id <> auth.uid() or v_row.account_id <> v_identity.account_id or v_row.sh_id <> v_identity.sh_id then
    raise exception 'HIGH_RISK_EXECUTION_REJECTED: actor/ownership boundary failed';
  end if;
  if v_row.operation <> 'RECOVERY_RESTORE' then raise exception 'HIGH_RISK_EXECUTION_REJECTED: unsupported operation'; end if;
  if not exists (
    select 1 from public.recovery_snapshots rs
    where rs.snapshot_id=v_row.target_id and rs.account_id=v_identity.account_id and rs.sh_id=v_identity.sh_id
  ) then raise exception 'HIGH_RISK_EXECUTION_REJECTED: target snapshot no longer accessible'; end if;
  ...
  perform public.runtime_record_audit(v_identity.sh_id,'RUNTIME_ACTION','SUCCESS',jsonb_build_object(
    'action','HIGH_RISK_ACTION_EXECUTED','action_id',v_row.action_id,'operation',v_row.operation,'confirmation_id',p_confirmation_id,'target_id',v_row.target_id,'recovery_event_id',v_recovery_event_id,'revalidated_at',now()
  ));
