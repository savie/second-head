...
with check (
  ...
  and exists (
    ...
    where o.account_id = auth.uid() and o.sh_id = audit_events.sh_id
  )
);
...
begin
  if auth.uid() is null then
    raise exception 'RUNTIME_AUDIT_REJECTED: authentication required';
  end if;
  ...
  where exists (
    select 1 from public.sh_ownership o
    where o.account_id = auth.uid() and o.sh_id = p_sh_id
  )
