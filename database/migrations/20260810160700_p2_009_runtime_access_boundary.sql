...
begin
  ...
  if v_auth_uid is null then
    return query select 'REJECT'::text, null::uuid, null::text,
  ...
  if p_actor_account_id is not null and p_actor_account_id <> v_account_id then
    return query select 'REJECT'::text, v_account_id, null::text,
  ...
  if v_relation not in ('SELF', 'SYSTEM') then
    return query select 'REJECT'::text, v_account_id, v_relation,
      case
