...
begin
  ...
  if v_missing_after > 0 then v_continuity_status := 'GAP_UNRESOLVED'; v_gap_code := 'CONTINUITY_GAP_UNRESOLVED';
  else v_continuity_status := 'RECOVERED'; v_gap_code := case when v_missing_before > 0 then 'CONTINUITY_GAP_RECOVERED' else null end;
  end if;
