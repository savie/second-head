...
as $$
declare v_new uuid; v_account uuid;
begin
  ...
  values(p_sh_id,'LONG_TERM',btrim(p_new_content),coalesce(nullif(btrim(p_source),''),'runtime:p5a:explicit_user_request'),p_scope,p_visibility,'CANDIDATE',1) returning memory_id into v_new;
  if nullif(btrim(p_old_pattern),'') is not null then
    update public.memories set lifecycle='UPDATED',superseded_by=v_new,updated_at=now() where sh_id=p_sh_id and lifecycle in ('CANDIDATE','ACTIVE') and superseded_by is null and content ilike '%' || btrim(p_old_pattern) || '%';
  end if;
  return v_new;
end;
$$;
