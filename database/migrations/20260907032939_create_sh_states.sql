...
using (
  exists (
    ...
    where si.sh_id = sh_states.sh_id
      and si.account_id = public.current_account_id()
  )
);
...
begin
  ...
  if not exists (
    select 1 from public.sh_instances
...
create or replace function public.runtime_mutate_sh_state(
  p_sh_id uuid,
...
begin
  ...
  if v_operation is null or v_operation not in ('SET','MERGE','REMOVE') then
    raise exception 'INVALID_MUTATION';
  end if;

  select revision, state_version, state_payload
    into v_current_revision, v_state_version, v_current_payload
  from public.sh_states
  where sh_id = p_sh_id
  for update;
  ...
  if jsonb_typeof(v_new_payload) <> 'object' then
    raise exception 'INVALID_STATE';
  end if;

  update public.sh_states
  set state_payload = v_new_payload,
