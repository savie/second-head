...
begin
  ...
  where m_sh.sh_id=v_auth.target_sh_id and m.provenance->'inheritance_origin'->>'authorization_id'=p_authorization_id::text
    and not exists (select 1 from public.journey_events j where j.sh_id=m.sh_id and j.event_type='MEMORY' and j.provenance->>'transfer_operation'='INHERITANCE' and j.provenance->>'authorization_id'=p_authorization_id::text and j.provenance->>'source_memory_id'=m.provenance->'inheritance_origin'->>'source_memory_id');
  insert into public.journey_events(sh_id,account_id,event_type,occurred_at,continuity_status,payload,source_ref,visibility,transfer_policy,provenance)
  ...
  where k_sh.sh_id=v_auth.target_sh_id and k.provenance->'inheritance_origin'->>'authorization_id'=p_authorization_id::text
    and not exists (select 1 from public.journey_events j where j.sh_id=k.sh_id and j.event_type='KNOWLEDGE' and j.provenance->>'transfer_operation'='INHERITANCE' and j.provenance->>'authorization_id'=p_authorization_id::text and j.provenance->>'source_knowledge_id'=k.provenance->'inheritance_origin'->>'source_knowledge_id');
  insert into public.journey_events(sh_id,account_id,event_type,occurred_at,continuity_status,payload,source_ref,visibility,transfer_policy,provenance)
  ...
  where e_sh.sh_id=v_auth.target_sh_id and e.provenance->'inheritance_origin'->>'authorization_id'=p_authorization_id::text
    and not exists (select 1 from public.journey_events j where j.sh_id=e.sh_id and j.event_type='EXPERIENCE' and j.provenance->>'transfer_operation'='INHERITANCE' and j.provenance->>'authorization_id'=p_authorization_id::text and j.provenance->>'source_experience_id'=e.provenance->'inheritance_origin'->>'source_experience_id');
  return v_result;
end; $$;

CREATE OR REPLACE FUNCTION public.runtime_execute_succession(p_succession_id uuid)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
declare v_rule public.succession_rules%rowtype; v_result uuid; v_target_sh_id uuid;
begin
  ...
  select sh_id into v_target_sh_id from public.sh_instances where account_id=v_rule.successor_account_id and status<>'deactivated' and is_primary=true order by created_at asc limit 1;
  if v_target_sh_id is null then raise exception 'SUCCESSION_REJECTED: active successor PRIMARY SH required'; end if;
  insert into public.journey_events(sh_id,account_id,event_type,occurred_at,continuity_status,payload,source_ref,visibility,transfer_policy,provenance)
  ...
  from public.memories m join public.sh_instances m_sh on m_sh.sh_id=m.sh_id where m_sh.sh_id=v_target_sh_id and m.provenance->'succession_origin'->>'succession_id'=p_succession_id::text
    and not exists (select 1 from public.journey_events j where j.sh_id=m.sh_id and j.event_type='MEMORY' and j.provenance->>'transfer_operation'='SUCCESSION' and j.provenance->>'succession_id'=p_succession_id::text and j.provenance->>'source_memory_id'=m.provenance->'succession_origin'->>'source_memory_id');
  insert into public.journey_events(sh_id,account_id,event_type,occurred_at,continuity_status,payload,source_ref,visibility,transfer_policy,provenance)
  ...
  from public.knowledge k join public.sh_instances k_sh on k_sh.sh_id=k.sh_id where k_sh.sh_id=v_target_sh_id and k.provenance->'succession_origin'->>'succession_id'=p_succession_id::text
    and not exists (select 1 from public.journey_events j where j.sh_id=k.sh_id and j.event_type='KNOWLEDGE' and j.provenance->>'transfer_operation'='SUCCESSION' and j.provenance->>'succession_id'=p_succession_id::text and j.provenance->>'source_knowledge_id'=k.provenance->'succession_origin'->>'source_knowledge_id');
  insert into public.journey_events(sh_id,account_id,event_type,occurred_at,continuity_status,payload,source_ref,visibility,transfer_policy,provenance)
  ...
  from public.experiences e join public.sh_instances e_sh on e_sh.sh_id=e.sh_id where e_sh.sh_id=v_target_sh_id and e.provenance->'succession_origin'->>'succession_id'=p_succession_id::text
    and not exists (select 1 from public.journey_events j where j.sh_id=e.sh_id and j.event_type='EXPERIENCE' and j.provenance->>'transfer_operation'='SUCCESSION' and j.provenance->>'succession_id'=p_succession_id::text and j.provenance->>'source_experience_id'=e.provenance->'succession_origin'->>'source_experience_id');
  return v_result;
end; $$;
