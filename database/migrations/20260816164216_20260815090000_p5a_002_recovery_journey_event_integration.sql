...
begin
  ...
  v_continuity_status := case when v_gap is null then 'RECOVERED' else 'GAP_UNRESOLVED' end;
  insert into public.recovery_events (snapshot_id, sh_id, outcome, continuity_status, gap_code) values (p_snapshot_id, v_sh_id, 'RESTORED', v_continuity_status, v_gap) returning recovery_event_id into v_recovery_event_id;
  insert into public.journey_events (sh_id, account_id, event_type, occurred_at, continuity_status, gap_code, payload, source_ref)
  values (v_sh_id, current_account_id(), 'RECOVERY', now(), v_continuity_status, v_gap, jsonb_build_object('outcome','RESTORED','snapshot_id',p_snapshot_id,'recovery_event_id',v_recovery_event_id), 'recovery_event:' || v_recovery_event_id::text);
