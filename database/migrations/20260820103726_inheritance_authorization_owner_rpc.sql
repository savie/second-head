-- Inheritance authorization creation must be source-owner initiated by the current DEV UX contract.
-- Keep the table RLS boundary intact; expose a narrow security-definer RPC that
-- validates source ownership and target identity before inserting the request.
...
$$;
revoke all on function public.runtime_create_inheritance_authorization(uuid,uuid,uuid,uuid,jsonb) from public;
