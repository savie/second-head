...
CREATE POLICY account_auth_links_select_own ON public.account_auth_links
FOR SELECT USING (provider = 'supabase' AND subject_ref = auth.uid()::text);
...
CREATE POLICY sh_instances_select_own ON public.sh_instances
FOR SELECT USING (account_id = public.current_account_id());

DROP POLICY IF EXISTS sh_ownership_select_own ON public.sh_ownership;
CREATE POLICY sh_ownership_select_own ON public.sh_ownership
FOR SELECT USING (account_id = public.current_account_id());
