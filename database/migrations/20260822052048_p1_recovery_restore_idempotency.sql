ALTER TABLE public.recovery_events ADD CONSTRAINT recovery_events_snapshot_id_unique UNIQUE (snapshot_id);

CREATE UNIQUE INDEX IF NOT EXISTS journey_recovery_source_ref_unique
  ON public.journey_events (source_ref)
  WHERE event_type = 'RECOVERY' AND source_ref IS NOT NULL;
...
begin
  ...
  for update;
  if found then
    return v_existing_event.recovery_event_id;
  end if;
  ...
  select coalesce((select count(*) from jsonb_to_recordset(coalesce(v_snapshot.manifest->'ownership_root','[]'::jsonb)) x(ownership_id uuid,account_id uuid,sh_id uuid) where x.sh_id=v_sh_id and x.account_id=public.current_account_id() and not exists(select 1 from public.sh_ownership o where o.ownership_id=x.ownership_id)),0)
