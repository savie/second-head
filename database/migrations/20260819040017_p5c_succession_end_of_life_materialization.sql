...
begin
  ...
  select * into v_rule
    from public.succession_rules
   where succession_id=p_succession_id
     and status='ACTIVE'
   for update;
  if not found then raise exception 'SUCCESSION_REJECTED: active succession rule required'; end if;

  select * into v_source from public.sh_instances
   where sh_id=v_rule.source_sh_id;
  ...
  select * into v_target from public.sh_instances
   where account_id=v_rule.successor_account_id
  ...
  insert into public.memories (
    sh_id,memory_type,content,source,confidence,scope,visibility,lifecycle,occurrence_count,created_at,updated_at,superseded_by
  )
  select
  ...
  insert into public.knowledge (
    content,knowledge_class,scope,visibility,source,provenance,confidence,version,lifecycle,superseded_by,created_at,updated_at,sh_id
  )
  select
    k.content,k.knowledge_class,k.scope,k.visibility,k.source,
    jsonb_build_object(
  ...
  insert into public.succession_events(
    succession_id,source_sh_id,target_account_id,target_sh_id,scope,transferred_counts,provenance
  ) values (
    v_rule.succession_id,v_source.sh_id,v_rule.successor_account_id,v_target.sh_id,v_scope,
  ...
  update public.succession_rules
     set status='CONSUMED'
   where succession_id=v_rule.succession_id;
