create or replace function public.runtime_transfer_selected_journey_events(p_operation text, p_source_sh_id uuid, p_target_sh_id uuid, p_event_ids uuid[]) returns integer language plpgsql security definer set search_path=public as $$ declare v_source_account uuid; v_target_account uuid; v_count integer; v_op text:=upper(p_operation); v_required_policy text; v_inheritance_authorization uuid;
begin
