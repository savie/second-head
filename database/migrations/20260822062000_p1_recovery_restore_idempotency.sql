-- P1 recovery idempotency reconciliation.
-- One recovery outcome per snapshot; repeated restore calls must not append duplicate
-- RESTORED recovery events or RECOVERY Journey events.
ALTER TABLE public.recovery_events
  ADD CONSTRAINT recovery_events_snapshot_id_unique UNIQUE (snapshot_id);
