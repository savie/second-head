...
begin
  ...
  select * into v_event from public.journey_events e where e.event_id = p_event_id;
  if not found then return false; end if;
  if upper(v_event.event_type) = 'MEMORY' then
    v_id := nullif(v_event.payload ->> 'memory_id', '')::uuid;
    if v_id is null then return false; end if;
    select (m.scope = 'GENERAL' and m.visibility = 'SHARED') into v_shared from public.memories m where m.memory_id = v_id and m.sh_id = v_event.sh_id;
  ...
  if upper(v_event.event_type) in ('LEARNING','KNOWLEDGE') then
    ...
    if v_id is null then return false; end if;
    select (k.scope = 'GENERAL' and k.visibility = 'SHARED') into v_shared from public.knowledge k where k.knowledge_id = v_id and k.sh_id = v_event.sh_id;
    return coalesce(v_shared, false);
  end if;
  if upper(v_event.event_type) = 'EXPERIENCE' then
    v_id := nullif(v_event.payload ->> 'experience_id', '')::uuid;
    if v_id is not null then
      select (x.scope = 'GENERAL' and x.visibility = 'SHARED') into v_shared from public.experiences x where x.experience_id = v_id and x.sh_id = v_event.sh_id;
      return coalesce(v_shared, false);
    end if;
    select count(*) = 1 and bool_and(x.scope = 'GENERAL' and x.visibility = 'SHARED') into v_shared from public.experiences x where x.sh_id = v_event.sh_id and x.source_ref = 'runtime:p5a:explicit_user_capture' and x.content = coalesce(v_event.payload ->> 'representation', '');
    return coalesce(v_shared, false);
  end if;
  return false;
end;
...
create policy journey_events_visibility_select
  ...
  using (
    exists (select 1 from public.sh_instances s where s.sh_id = journey_events.sh_id and s.account_id = public.current_account_id())
