...
-- BL-P1-002 already created UNIQUE(provider, subject_ref); do not recreate it.
...
AS $$
BEGIN
  PERFORM public.provision_identity_for_auth_subject(NEW.id::text, NEW.email);
  RETURN NEW;
END;
$$;
...
BEGIN
  FOR v_user IN
    SELECT id, email FROM auth.users ORDER BY created_at, id
  LOOP
    PERFORM public.provision_identity_for_auth_subject(v_user.id::text, v_user.email);
  END LOOP;
END;
$$;
