-- SECOND HEAD — Corrective migration for conversation attachment finalize.
-- Root cause: RETURNS TABLE output columns shadow unqualified table columns in PL/pgSQL.
-- This migration qualifies the lookup columns without changing the contract.
...
BEGIN
  ...
  SET message_id = p_message_id,
      ...
      persisted_at = now(),
      updated_at = now()
  WHERE conversation_attachments.attachment_id = v_attachment.attachment_id;
