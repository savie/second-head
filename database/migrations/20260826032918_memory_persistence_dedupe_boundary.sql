create or replace function public.runtime_record_memory_with_journey(p_sh_id uuid, p_content text, p_memory_type text default 'LONG_TERM', p_source text default 'runtime_response', p_confidence numeric default null, p_scope text default 'PRIVATE', p_visibility text default 'OWNER_ONLY', p_lifecycle text default 'CANDIDATE') returns uuid language plpgsql security definer set search_path to 'public' as $function$
declare
  v_memory_id uuid;
  v_account_id uuid;
...
begin
  ...
  v_normalized_content := lower(regexp_replace(btrim(p_content), '\\s+', ' ', 'g'));

  select memory_id into v_memory_id
    from public.memories
   where sh_id=p_sh_id
     and lower(regexp_replace(btrim(content), '\\s+', ' ', 'g'))=v_normalized_content
  ...
  if v_memory_id is not null then
    ...
    insert into public.memories(sh_id,memory_type,content,source,confidence,scope,visibility,lifecycle,occurrence_count)
    values(p_sh_id,p_memory_type,btrim(p_content),p_source,p_confidence,p_scope,p_visibility,p_lifecycle,1)
    returning memory_id into v_memory_id;
  end if;

  perform public.runtime_record_journey_event(
    p_sh_id,'MEMORY',now(),'CONTINUOUS',null,
    jsonb_build_object('memory_id',v_memory_id,'content',btrim(p_content),'memory_type',p_memory_type,'scope',p_scope,'visibility',p_visibility,'lifecycle',p_lifecycle,'acquisition','MODEL_OR_EXPLICIT_CANDIDATE'),
    coalesce(nullif(btrim(p_source),''),'runtime:p4d:memory_candidate')
  );
  return v_memory_id;
end;
$function$;