...
as $$
  select m.memory_id,m.sh_id,m.memory_type,m.content,m.source,m.confidence,m.scope,m.visibility,m.lifecycle,m.occurrence_count,m.created_at,m.updated_at,m.superseded_by,public.memory_relevance_score(p_query_text,m.content)
  ...
  order by public.memory_relevance_score(p_query_text,m.content) desc,m.updated_at desc,m.occurrence_count desc,m.memory_id asc
