create or replace function public.list_experiences(p_sh_id uuid default null, p_limit integer default 50)
returns table (
  experience_id uuid,
  sh_id uuid,
  account_id uuid,
...
begin
  ...
  select e.* into v from public.experiences e
...
comment on function public.list_experiences(uuid,integer) is 'Owner-scoped Experience retrieval. Cross-account and cross-owner access is rejected by the account boundary.';
