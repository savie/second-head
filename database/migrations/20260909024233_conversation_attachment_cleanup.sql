...
CREATE TABLE public.conversation_attachment_cleanup_queue (
  ...
  CONSTRAINT conversation_attachment_cleanup_status_check CHECK (status IN ('QUEUED','PROCESSING','FAILED','COMPLETED')),
  CONSTRAINT conversation_attachment_cleanup_attempts_check CHECK (attempts >= 0)
);

CREATE INDEX conversation_attachment_cleanup_ready_idx ON public.conversation_attachment_cleanup_queue(status, available_at, created_at);
CREATE INDEX conversation_attachment_cleanup_account_sh_idx ON public.conversation_attachment_cleanup_queue(account_id, sh_id, status, available_at);
CREATE UNIQUE INDEX conversation_attachment_cleanup_attachment_uidx ON public.conversation_attachment_cleanup_queue(attachment_id) WHERE attachment_id IS NOT NULL AND status <> 'COMPLETED';
...
BEGIN
  IF OLD.message_id IS NOT NULL AND NEW.message_id IS NULL THEN
    INSERT INTO public.conversation_attachment_cleanup_queue(attachment_id, account_id, sh_id, storage_ref, status, attempts, available_at, locked_at, last_error, updated_at)
    VALUES(NEW.attachment_id, NEW.account_id, NEW.sh_id, NEW.storage_ref, 'QUEUED', 0, now(), NULL, NULL, now())
    ON CONFLICT (attachment_id) WHERE attachment_id IS NOT NULL AND status <> 'COMPLETED'
    DO UPDATE SET storage_ref=EXCLUDED.storage_ref, account_id=EXCLUDED.account_id, sh_id=EXCLUDED.sh_id, status='QUEUED', available_at=now(), locked_at=NULL, last_error=NULL, updated_at=now();
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS conversation_attachment_cleanup_enqueue ON public.conversation_attachments;
...
BEGIN
  ...
  FOR v_queue IN
    ...
    WHERE q.account_id=v_identity.account_id AND q.sh_id=v_identity.sh_id
  ...
  LOOP
    ...
    WHERE a.attachment_id=v_queue.attachment_id AND a.account_id=v_identity.account_id AND a.sh_id=v_identity.sh_id FOR UPDATE;
    ...
    IF v_attachment.message_id IS NOT NULL THEN
      UPDATE public.conversation_attachment_cleanup_queue SET status='COMPLETED',locked_at=NULL,last_error=NULL,updated_at=now() WHERE cleanup_id=v_queue.cleanup_id;
      CONTINUE;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM public.conversation_attachment_recovery_refs r WHERE r.attachment_id=v_attachment.attachment_id) THEN
      UPDATE public.conversation_attachment_cleanup_queue SET status='QUEUED',available_at=now()+interval '15 minutes',locked_at=NULL,last_error='RECOVERY_DEPENDENCY_PRESENT',updated_at=now() WHERE cleanup_id=v_queue.cleanup_id;
      CONTINUE;
    END IF;

    IF v_attachment.status <> 'PERSISTED' THEN
      UPDATE public.conversation_attachment_cleanup_queue SET status='QUEUED',available_at=now()+interval '15 minutes',locked_at=NULL,last_error='NON_PERSISTED_ATTACHMENT_DEFERRED',updated_at=now() WHERE cleanup_id=v_queue.cleanup_id;
      CONTINUE;
    END IF;
