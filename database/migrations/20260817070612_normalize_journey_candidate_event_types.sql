create or replace function public.runtime_record_journey_event(p_sh_id uuid, p_event_type text, p_occurred_at timestamptz default now(), p_continuity_status text default 'CONTINUOUS', p_gap_code text default null, p_payload jsonb default '{}'::jsonb, p_source_ref text default null) returns uuid language plpgsql set search_path=public as $function$
declare
  v_event_id uuid;
  v_account_id uuid;
...
begin
  ...
  if v_event_type not in ('LIFECYCLE','EXPERIENCE','MEMORY','LEARNING','EVOLUTION','MIGRATION','RECOVERY','CONTINUITY','SHARING','INHERITANCE','LEGACY') then
    if v_event_type in ('DECISION','COMMITMENT','CONTINUATION','TRANSITION','MILESTONE','STATE_CHANGE','STATE-CHANGE')
