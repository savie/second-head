...
const journeyEventId = await rpc('runtime_record_journey_event', {
  ...
  p_gap_code: null,
  p_payload: { recovery_event_id: recoveryEventId, outcome: 'RESTORED' },
...
const event = after[0];
if (event.event_id !== journeyEventId || event.event_type !== 'RECOVERY' || event.continuity_status !== 'RECOVERED' || event.sh_id !== shId || event.payload?.recovery_event_id !== recoveryEventId) {
...
console.log(JSON.stringify({
  status: 'PASS',
  mode: 'AUTHENTICATED_RUNTIME_INTEGRATION',
  note: 'This verifies the authenticated Recovery RPC result and canonical Journey recorder contract. The React Native client handler itself is statically wired in recovery-service.ts and is not executed by this Node-only CI step.',
  ...
  assertions: [
    'authenticated test account resolved',
    'owned SH resolved',
    'Recovery restored with RECOVERED outcome',
    'canonical RECOVERY Journey recorder accepted the event',
    'exactly one RECOVERY/RECOVERED event exists for the recovery_event_id',
    'Journey event is scoped to the recovered SH',
  ],
}, null, 2));
