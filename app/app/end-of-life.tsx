...
export default function EndOfLifeScreen() {
  ...
  if (!context || !current) return <ActivityIndicator />;
  ...
  async function confirmEndOfLife() {
    ...
    setNotice(null);
    setError(null);
    const { error: rpcError } = await supabase.rpc('runtime_end_of_life_sh', {
      p_sh_id: currentShId,
      p_reason: 'Owner confirmed End-of-Life from Second Head app',
    });
    if (rpcError) {
      setError(`END_OF_LIFE_FAILED: ${rpcError.message}`);
    } else {
      setNotice('End-of-Life completed. Account and SH are now deactivated.');
      await logout();
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 24, gap: 14 }}>
      ...
      <Text>{context.account.account_id}</Text>
      <Text style={{ fontWeight: '600' }}>SH</Text>
      <Text>{currentShId}</Text>
      <Text style={{ fontWeight: '600' }}>Email</Text>
      <Text>{context.account.email}</Text>
      <Text style={{ fontWeight: '600' }}>Current status</Text>
      <Text>{context.account.status} / {current.status}</Text>
