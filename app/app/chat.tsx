...
export default function ChatScreen() {
  ...
  const [journeyCaptureState, setJourneyCaptureState] = useState<'idle' | 'saving' | 'saved' | 'error'>('idle');
  ...
  async function onSend() {
    ...
    if (!message || sending || pendingConfirmation || lifecycleState === 'background') return;
    ...
    try {
      await streamSHRuntime(
        ...
        (event) => {
          ...
          if (event.type === 'complete') setLifecycleState('idle');
        },
        controller.signal,
      );
    } catch (error) {
      ...
      if (controller.signal.aborted) {
        ...
        setMessages((current) => [...current, 'SH: Streaming cancelled because the App left the foreground or the request was cancelled.']);
        return;
      }
      ...
      if (mountedRef.current) {
        setSending(false);
        setLifecycleState((current) => current === 'streaming' ? 'idle' : current);
      }
    }
  }
  ...
  return (
    <View style={{ flex: 1, padding: 20, gap: 12 }}>
      ...
      <Text>App → authenticated Runtime → lifecycle-aware streaming</Text>
      <Text>State: {lifecycleState}</Text>
      ...
      {lastUserMessage && !sending ? (
        <View style={{ gap: 6 }}>
          <Button title={journeyCaptureState === 'saving' ? 'Saving to Journey...' : 'SAVE LAST MESSAGE TO JOURNEY'} onPress={() => void saveLastMessageToJourney()} disabled={journeyCaptureState === 'saving'} />
          {journeyCaptureState === 'saved' ? <Text>Journey event saved.</Text> : null}
          {journeyCaptureState === 'error' ? <Text>Journey capture failed.</Text> : null}
        </View>
      ) : null}
      ...
      {confirmationState !== 'idle' ? (
        <Text accessibilityRole="text">
          {confirmationState === 'cancelled' ? 'High-risk confirmation cancelled.' : 'Explicit confirmation recorded; no App-side authorization or execution occurred.'}
        </Text>
      ) : null}
      {sending ? <Button title="Cancel streaming" onPress={cancelStreaming} /> : null}

      <Text style={{ fontWeight: '600' }}>Pesan untuk SH</Text>
      <TextInput
        value={draft}
        onChangeText={setDraft}
        placeholder="Tulis pesan yang ingin dikirim ke SH..."
        placeholderTextColor="#6B7280"
        multiline
        editable={!sending && lifecycleState === 'active'}
        style={inputStyle}
      />
      <Button title={sending ? 'Streaming...' : 'Kirim'} onPress={() => void onSend()} disabled={!canSend} />
    </View>
  );
}
