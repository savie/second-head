...
export default function CloneScreen() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    setError(null);
    try { setAgreements(await listCloneAgreements()); }
    catch (err) { setError(describeError(err, 'Unable to load clone agreements')); }
  ...
  async function requestClone() {
    ...
    try {
      ...
      setSourceShId(''); setRecipientEmail('');
      setNotice(`Clone invitation created: ${agreement.agreement_id}`);
      await refresh();
    } catch (err) { setError(describeError(err, 'Unable to create clone invitation')); }
    finally { setBusyId(null); }
  }

  async function approve(agreementId: string) {
    setBusyId(agreementId); setError(null);
    try { await approveCloneAgreement(agreementId); setNotice(`Clone invitation approved: ${agreementId}`); await refresh(); }
    catch (err) { setError(describeError(err, 'Unable to approve clone invitation')); }
    finally { setBusyId(null); }
  }

  async function reject(agreementId: string) {
    setBusyId(agreementId); setError(null);
    try { await rejectCloneAgreement(agreementId); setNotice(`Clone invitation rejected: ${agreementId}`); await refresh(); }
    catch (err) { setError(describeError(err, 'Unable to reject clone invitation')); }
    finally { setBusyId(null); }
  }
    ...
    <ScrollView contentContainerStyle={{ padding: 24, gap: 16 }}>
      ...
      {incoming.length === 0 ? <Text>No incoming Clone invitations.</Text> : incoming.map((agreement) => (
        <View key={agreement.agreement_id} style={{ borderWidth: 1, padding: 12, borderRadius: 8, gap: 8 }}>
          <Text>Agreement: {agreement.agreement_id}</Text><Text>Source SH: {agreement.source_sh_id}</Text><Text>Source account: {agreement.source_account_id}</Text><Text>Recipient email: {agreement.target_email}</Text><Text>Status: {agreement.status}</Text>
          {agreement.status === 'APPROVED' && !agreement.target_account_id ? <Text>Approved. Register with the intended email to materialize this Clone automatically as the PRIMARY SH.</Text> : null}
      ...
      {outgoing.length === 0 ? <Text>No outgoing Clone invitations.</Text> : outgoing.map((agreement) => (
        <View key={agreement.agreement_id} style={{ borderWidth: 1, padding: 12, borderRadius: 8, gap: 8 }}>
          <Text>Agreement: {agreement.agreement_id}</Text><Text>Recipient email: {agreement.target_email}</Text><Text>Target account: {agreement.target_account_id ?? 'Not registered yet'}</Text><Text>Status: {agreement.status}</Text>
          {agreement.status === 'PENDING' ? <View style={{ gap: 8 }}><Button title={busyId === agreement.agreement_id ? 'Working…' : 'Approve'} onPress={() => void approve(agreement.agreement_id)} disabled={busyId !== null} /><Button title="Reject" onPress={() => void reject(agreement.agreement_id)} disabled={busyId !== null} /></View> : null}
