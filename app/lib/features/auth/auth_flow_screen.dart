...
class _AuthFlowScreenState extends State<AuthFlowScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_mode == AuthMode.signUp) {
      if (_fullName.text.trim().isEmpty || _confirmation.text.isEmpty) {
    ...
    try {
      if (_mode == AuthMode.signIn) {
        ...
        await AuthSession.service.signUp(
          email: _email.text,
          password: _password.text,
      ...
      if (mounted) _show('Authentication gagal: $error');
    } finally {
  ...
  Future<void> _google() async {
    setState(() => _loading = true);
    try {
      await AuthSession.service.signInWithGoogle();
    } on AuthException catch (error) {
      if (mounted) _show(error.message);
    } catch (error) {
      if (mounted) _show('Google sign in gagal: $error');
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
  ...
  Future<void> _forgot() async {
    if (_email.text.trim().isEmpty) {
      _show('Email wajib diisi.');
      return;
    }
    setState(() => _loading = true);
    try {
      await AuthSession.service.sendPasswordReset(_email.text);
      if (mounted) _show('Reset link sudah dikirim jika email terdaftar.');
    } on AuthException catch (error) {
      if (mounted) _show(error.message);
    } catch (error) {
      if (mounted) _show('Gagal mengirim reset link: $error');
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: shBackground,
                      border: Border.all(
                        color: shElectric.withValues(alpha: .32),
                        width: 1.5,
                      ),
                      boxShadow: [
                        BoxShadow(
                          color: shElectric.withValues(alpha: .10),
                          blurRadius: 28,
                          spreadRadius: 4,
                        ),
                      ],
                    ),
                    child: const Center(child: ShBrandMark()),
                  ),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: shElectric.withValues(alpha: .18),
                      ),
                    ),
                    child: Column(
                      children: [
...
class _FieldState extends State<_Field> {
  ...
  Widget build(BuildContext context) => TextField(
        ...
        decoration: InputDecoration(
          prefixIcon: Icon(widget.icon, color: shMuted),
