...
class _RuntimeJourneyVisibilityDetailState extends State<RuntimeJourneyVisibilityDetail> {
  ...
  Future<void> _setVisibility(bool privateOnly) async {
    ...
    try {
      ...
      widget.onChanged();
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(privateOnly ? 'Visibility changed to Owner Only.' : 'Visibility changed to Shared.')));
    } catch (error) {
