...
class JourneyRuntimeService {
  ...
  Future<String> createMemory({required String shId, required String content, String memoryType = 'LONG_TERM', String source = 'journey-ui', double confidence = 1, String scope = 'PRIVATE', String visibility = 'OWNER_ONLY', String lifecycle = 'CANDIDATE'}) async {
  ...
  Future<void> setTransferPolicy({required String domain, required String recordId, required String transferPolicy}) async {
    await backendClient.rpc('runtime_set_transfer_policy', params: {'p_domain': domain, 'p_record_id': recordId, 'p_transfer_policy': transferPolicy});
  }
  ...
  Future<String> prepareSelectedTransferAsLegacy({required String sourceShId, required Map<String, dynamic> scope}) async => _uuid(await backendClient.rpc('runtime_prepare_selected_transfer_as_legacy', params: {'p_source_sh_id': sourceShId, 'p_scope': scope}), 'Legacy preparation');
  Future<String> activatePreparedLegacy({required String sourceShId}) async => _uuid(await backendClient.rpc('runtime_activate_prepared_legacy', params: {'p_source_sh_id': sourceShId}), 'Legacy activation');
  Future<String> preserveSelectedTransferAsLegacy({required String sourceShId, required Map<String, dynamic> scope}) async => _uuid(await backendClient.rpc('runtime_preserve_selected_transfer_as_legacy', params: {'p_source_sh_id': sourceShId, 'p_scope': scope}), 'Selected legacy preservation');
