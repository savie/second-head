...
class _SideMenuState extends State<SideMenu> {
  ...
  Future<void> _startNewConversation({String? projectId}) async {
    try {
      final id = await _runtime.createConversation(
        projectId: projectId,
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              Icon(icon, size: 18, color: shCyan),
              const SizedBox(width: 9),
              Text(label, style: const TextStyle(fontSize: 11, color: shCyan)),
            ],
          ),
        ),
      ),
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const Padding(
            padding: EdgeInsets.only(left: 38, top: 3, bottom: 4),
            child: Align(
          ...
          for (final project in _projects)
            ListTile(
              ...
              trailing: Text(
                '${_conversations.where((c) => c.projectId == project.projectId).length}',
                      ...
                      builder: (context, name, _) => ValueListenableBuilder<String>(
                        ...
                        builder: (context, email, _) => Column(
                          ...
                          children: [
                            ...
                            Text(email, style: const TextStyle(fontSize: 9, color: shMuted)),
                          ],
                        ),
                      ),
                    ),
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  MenuTile(icon: Icons.help_outline, label: 'Help & Support', onTap: () {
                    _closeDrawer();
                    Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const HelpSupportView()));
                  }),
                  MenuTile(icon: Icons.info_outline, label: 'About', onTap: () {
                    _closeDrawer();
                    Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const AboutView()));
                  }),
                ],
              ),
            ),
