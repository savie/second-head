...
class _LifecycleAuthorityViewState extends State<LifecycleAuthorityView> {
  ...
  String get _requiredTransferPolicy => switch (widget.stage.title) {'Inheritance' => 'INHERITANCE', 'Succession' => 'SUCCESSION', _ => 'NON_TRANSFERABLE'};
  ...
  Future<String?> _resolveCurrentAccountId() async {
    ...
    return (result.first as Map)['account_id']?.toString();
  }
    ...
    try {
      ...
      final incoming = [
        for (final record in journeyRecords)
          if (record.eventId.isNotEmpty && record.eventType.isNotEmpty && (record.visibility == 'SHARED' || record.visibility == 'PUBLIC') && const {'MEMORY', 'KNOWLEDGE', 'LEARNING', 'EXPERIENCE'}.contains(record.eventType.toUpperCase()) && (!_isTransfer || record.transferPolicy == _requiredTransferPolicy))
      ...
      if (mounted) setState(() { _records = requestRows; _journeyItems = incoming; _currentAccountId = accountId; });
  ...
  Future<void> _submit() async {
    ...
    for (final target in _targets) {
      ...
      if (_isTransfer && target.selected.isEmpty) { _show(StateError('Select at least one eligible shared Journey item.')); return; }
    }
    setState(() => _busy = true);
    try {
      for (final target in _targets) {
  ...
  bool _canExecute(Map<String, dynamic> record) {
    ...
    if (widget.stage.title == 'Inheritance') return status == 'APPROVED' && record['source_account_id']?.toString() == _currentAccountId;
    if (widget.stage.title == 'Succession') return status == 'ACTIVE' && record['successor_account_id']?.toString() == _currentAccountId;
    return (widget.stage.title == 'Clone') && status == 'APPROVED';
  }
  ...
  Future<void> _execute(Map<String, dynamic> record) async {
    ...
    final allowedStatus = (_isClone || widget.stage.title == 'Inheritance') ? status == 'APPROVED' : status == 'ACTIVE';
    if (id == null || id.isEmpty || !allowedStatus || !_canExecute(record)) return;
  ...
  Widget _target() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_isClone ? 'Target & Journey' : 'Target & Incoming from Journey', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w500)), const SizedBox(height: 18), for (var i = 0; i < _targets.length; i++) ...[Text('Target ${i + 1}', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500)), const SizedBox(height: 9), const Text('Email', style: TextStyle(fontSize: 12, color: shMuted)), const SizedBox(height: 6), TextField(controller: _targets[i].email, keyboardType: TextInputType.emailAddress, decoration: const InputDecoration(prefixIcon: Icon(Icons.mail_outline_rounded), hintText: 'Enter target email')), if (_hasJourneyChecklist) ...[const SizedBox(height: 15), Text(_isClone ? 'Shared Journey' : 'Eligible shared Journey (${_requiredTransferPolicy})', style: const TextStyle(fontSize: 12, color: shMuted)), const SizedBox(height: 6), if (_journeyItems.isEmpty) const Text('No eligible shared Journey data available. Set an explicit transfer policy on the Journey item first.', style: TextStyle(fontSize: 13, color: shMuted)) else for (final item in _journeyItems) CheckboxListTile(contentPadding: EdgeInsets.zero, value: _isClone ? true : _targets[i].selected.contains(item.semanticSourceId ?? '${item.type}|${item.title}'), onChanged: _busy || _isClone ? null : (_) => _toggle(i, item), title: Text(item.title, style: const TextStyle(fontSize: 14)), subtitle: Text('${item.type} · ${item.date} · ${item.transferPolicy}', style: const TextStyle(fontSize: 11, color: shMuted)), controlAffinity: ListTileControlAffinity.leading)], if (!_isClone && _targets.length > 1) Align(alignment: Alignment.centerRight, child: TextButton.icon(onPressed: _busy ? null : () => _removeTarget(i), icon: const Icon(Icons.remove_circle_outline, size: 18), label: const Text('Remove target'))), if (i != _targets.length - 1) const SizedBox(height: 15)], if (!_isClone) ...[const SizedBox(height: 9), OutlinedButton.icon(onPressed: _busy ? null : _addTarget, icon: const Icon(Icons.add_rounded, size: 18), label: const Text('Add Target'))], const SizedBox(height: 15), Text(_isClone ? 'Target email is used for the clone authorization flow. Authorization is handled by Integrations.' : 'Request ${widget.stage.title.toLowerCase()} using selected eligible shared Journey context.', style: const TextStyle(fontSize: 13, color: shMuted, height: 1.4)), const SizedBox(height: 15), SizedBox(width: double.infinity, height: 48, child: FilledButton.icon(onPressed: _busy ? null : _submit, icon: Icon(_isClone ? Icons.copy_all_outlined : Icons.send_rounded, size: 18), label: Text(_busy ? 'Submitting…' : _isClone ? 'Create Clone' : 'Request ${widget.stage.title}', style: const TextStyle(fontSize: 14)))), const SizedBox(height: 18), _summaryRow()]);
