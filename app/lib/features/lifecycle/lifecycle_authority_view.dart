...
class _LifecycleAuthorityViewState extends State<LifecycleAuthorityView> {
  ...
  Map<String, dynamic> _parseScope() {
    ...
    throw FormatException('Scope must be provided as canonical JSON object.');
  }
  ...
  String _recordTitle(Map<String, dynamic> record) {
    return '${record['status'] ?? 'UNKNOWN'} • ${record['agreement_id'] ?? record['authorization_id'] ?? record['succession_id'] ?? ''}';
  }
