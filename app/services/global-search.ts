...
export async function globalSearch(input: { shId: string; query: string; limit?: number; offset?: number; domains?: GlobalSearchDomain[] }): Promise<GlobalSearchPage> {
  ...
  const { data, error } = await backend.rpc('global_search_bounded', { p_sh_id: input.shId, p_query_text: query, p_limit: limit + 1, p_offset: offset, p_domains: input.domains?.length ? input.domains : null });
