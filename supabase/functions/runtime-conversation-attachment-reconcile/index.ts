...
function unauthorized() {
  return new Response(JSON.stringify({ error: "CONVERSATION_ATTACHMENT_RECONCILIATION_UNAUTHORIZED" }), {
    status: 401,
...
Deno.serve(async (req: Request) => {
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    ...
    for (const item of data ?? []) {
      if (item.id !== null && item.name) objectRefs.push(item.name);
    }
  ...
  for (let i = 0; i < objectRefs.length; i += 500) {
    ...
    const { data, error } = await admin.rpc("runtime_reconcile_conversation_attachment_storage_refs_internal", {
      p_storage_refs: refs,
    });
    if (error) {
      return new Response(JSON.stringify({ error: error.message }), {
  ...
  const { data: resources, error: resourceError } = await admin
    ...
    .select("attachment_id,storage_ref,status,message_id");
  if (resourceError) {
  ...
  for (const resource of resources ?? []) {
    if ((resource.status === "PENDING" || resource.status === "FAILED") && !objectRefSet.has(resource.storage_ref)) {
      retryableWithoutObject.push(resource.storage_ref);
    }
    if (resource.status === "PERSISTED" && !objectRefSet.has(resource.storage_ref)) {
      missingPersistedObject.push(resource.storage_ref);
    }
  }
  ...
  for (let i = 0; i < orphanRefs.length; i += DELETE_BATCH_SIZE) {
    ...
    for (const item of data ?? []) {
      if (item.name) deleted.push(item.name);
    }
  }
  ...
  return new Response(JSON.stringify({
    ...
    bucket: BUCKET,
    scanned_objects: objectRefs.length,
    orphan_objects_found: orphanRefs.length,
    orphan_objects_deleted: deleted.length,
    retryable_with_object: retryableWithObject.length,
    retryable_without_object: retryableWithoutObject.length,
    persisted_missing_object: missingPersistedObject.length,
    referenced_or_retained: referenced.length,
    delete_errors: deleteErrors,
  }), {
    status: deleteErrors.length === 0 ? 200 : 500,
    headers: { "Content-Type": "application/json" },
  });
});
