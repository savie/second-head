...
alter table public.memories add column if not exists transfer_policy text not null default 'NON_TRANSFERABLE';
alter table public.knowledge add column if not exists transfer_policy text not null default 'NON_TRANSFERABLE';
alter table public.experiences add column if not exists transfer_policy text not null default 'NON_TRANSFERABLE';

alter table public.memories drop constraint if exists memories_transfer_policy_check;
alter table public.memories add constraint memories_transfer_policy_check check (transfer_policy in ('NON_TRANSFERABLE','INHERITABLE','SUCCESSION','LEGACY'));
alter table public.knowledge drop constraint if exists knowledge_transfer_policy_check;
alter table public.knowledge add constraint knowledge_transfer_policy_check check (transfer_policy in ('NON_TRANSFERABLE','INHERITABLE','SUCCESSION','LEGACY'));
