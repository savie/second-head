...
-- The table INSERT policy remains target-account scoped. The owner creates the
-- authorization through this SECURITY DEFINER RPC after explicit ownership and
-- target validation; approval and execution remain separate operations.
...
begin
  ...
  if not exists (
    ...
    where s.sh_id = p_source_sh_id
      and s.account_id = p_source_account_id
  ...
  if not exists (
    ...
    where s.sh_id = p_target_sh_id
      and s.account_id = p_target_account_id
      and s.status <> 'deactivated'
  ) then
