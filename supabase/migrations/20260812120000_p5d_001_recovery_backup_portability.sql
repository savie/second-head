create table if not exists public.recovery_snapshots (
  ...
  snapshot_kind text not null default 'FULL' check (snapshot_kind in ('FULL','METADATA')),
...
create table if not exists public.recovery_events (
  ...
  snapshot_id uuid not null references public.recovery_snapshots(snapshot_id) on delete restrict,
  sh_id uuid not null references public.sh_instances(sh_id) on delete restrict,
  outcome text not null check (outcome in ('VALIDATED','RESTORED','FAILED')),
  continuity_status text not null check (continuity_status in ('CONTINUOUS','GAP_DETECTED','GAP_UNRESOLVED','RECOVERED')),
  gap_code text,
  created_at timestamptz not null default now()
);

create table if not exists public.portability_exports (
  export_id uuid primary key default gen_random_uuid(),
  snapshot_id uuid not null references public.recovery_snapshots(snapshot_id) on delete restrict,
  sh_id uuid not null references public.sh_instances(sh_id) on delete restrict,
  format text not null check (format in ('JSON')),
  manifest jsonb not null default '{}'::jsonb,
  status text not null default 'READY' check (status in ('READY','IMPORTED','REVOKED')),
  created_at timestamptz not null default now(),
  imported_at timestamptz
);

create index if not exists recovery_snapshots_sh_idx on public.recovery_snapshots(sh_id,created_at desc);
create index if not exists recovery_events_sh_idx on public.recovery_events(sh_id,created_at desc);
create index if not exists portability_exports_sh_idx on public.portability_exports(sh_id,created_at desc);
...
alter table public.portability_exports enable row level security;

create policy recovery_snapshots_owner_select on public.recovery_snapshots for select using (account_id=current_account_id());
create policy recovery_events_owner_select on public.recovery_events for select using (exists(select 1 from public.sh_instances s where s.sh_id=recovery_events.sh_id and s.account_id=current_account_id()));
create policy portability_exports_owner_select on public.portability_exports for select using (exists(select 1 from public.sh_instances s where s.sh_id=portability_exports.sh_id and s.account_id=current_account_id()));
...
create or replace function public.runtime_create_portability_export(p_snapshot_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public
as $$
declare v_snapshot recovery_snapshots%rowtype; v_export_id uuid;
begin
 select * into v_snapshot from public.recovery_snapshots where snapshot_id=p_snapshot_id and account_id=current_account_id();
 if not found then raise exception 'PORTABILITY_REJECTED: snapshot not accessible'; end if;
 insert into public.portability_exports(snapshot_id,sh_id,format,manifest) values(p_snapshot_id,v_snapshot.sh_id,'JSON',v_snapshot.manifest) returning export_id into v_export_id;
 return v_export_id;
end;
$$;
